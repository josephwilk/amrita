defmodule Amrita.Runner do
  ...
  defp spawn_cases(config, cases) do
    Enum.each cases, fn c ->
      all_modules = List.concat(c.__submodules__||[], [c])
      #IO.inspect all_modules
      Enum.each all_modules, spawn_case(config, &1)
    end
  ...
  defp run_test_case(config, pid, case_name) do
    ...
    { case_pid, case_ref } = Process.spawn_monitor fn ->
      ...
      #IO.inspect case_name
      tests = tests_for(case_name)
      #IO.inspect "tests for #{case_name}:  #{inspect(tests)}"
      #IO.inspect tests
  ...
  defp tests_for(case_name) do
    ...
    #IO.puts "Functions for #{case_name} are: #{inspect(exports)}"

    lc { function, 1 } inlist exports, is_test?(atom_to_list(function)) do
      #IO.puts "fns -> #{case_name} -> #{function}"
